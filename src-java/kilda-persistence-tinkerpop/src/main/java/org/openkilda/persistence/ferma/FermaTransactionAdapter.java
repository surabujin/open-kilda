...
public class FermaTransactionAdapter extends TransactionAdapter {
    private final DelegatingFramedGraph<?> graph;

    public FermaTransactionAdapter(TransactionArea area, DelegatingFramedGraph<?> graph) {
        super(area);
        this.graph = graph;
    }
    ...
    private void commitOrRollback(boolean isSuccess) throws Exception {
        commitOrRollback(graph.tx(), isSuccess);
    }
