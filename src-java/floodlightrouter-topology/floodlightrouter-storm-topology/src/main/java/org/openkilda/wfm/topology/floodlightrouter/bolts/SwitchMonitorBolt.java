...
public class SwitchMonitorBolt extends AbstractBolt implements SwitchMonitorCarrier {
    ...
    public void regionUpdateNotification(RegionMappingUpdate mappingUpdate) {
        getOutput().emit(STREAM_REGION_MAPPING_ID, getCurrentTuple(), makeRegionMappingTuple(mappingUpdate));
    }
    ...
    public void declareOutputFields(OutputFieldsDeclarer output) {
        ...
        output.declareStream(STREAM_NETWORK_ID, kafkaProducerFields);

        output.declareStream(STREAM_REGION_MAPPING_ID, STREAM_REGION_MAPPING_FIELDS);
    }
}
