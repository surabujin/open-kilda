...
import org.openkilda.wfm.topology.floodlightrouter.model.SwitchToRegionMapping;
...
public class ControllerToSpeakerProxyBolt extends AbstractBolt {
    ...
    protected transient SwitchToRegionMapping regionMapping;
    ...
    private void handleSwitchMappingUpdate(Tuple input) throws PipelineException {
        regionMapping.update(pullValue(input, SwitchMonitorBolt.FIELD_ID_PAYLOAD, RegionMappingUpdate.class));
    }
    ...
    private void handleControllerRequest(Object message) throws PipelineException {
        if (message instanceof CommandMessage && RouterUtils.isBroadcast(((CommandMessage) message).getData())) {
            ...
            handleDirectRequest(message);
        }
    }

    private void handleBroadcastRequest(CommandMessage message) throws PipelineException {
        Map<String, Set<SwitchId>> population = regionMapping.organizeReadWritePopulationPerRegion();
    ...
    private void handleDirectRequest(Object payload) throws PipelineException {
        SwitchId switchId = lookupSwitchId(payload);
        if (switchId != null) {
            Optional<String> region = regionMapping.lookupReadWriteRegion(switchId);
            if (region.isPresent()) {
    ...
    protected void init() {
        regionMapping = new SwitchToRegionMapping();
    }
