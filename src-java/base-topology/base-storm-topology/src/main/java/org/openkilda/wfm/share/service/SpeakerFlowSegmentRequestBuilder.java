...
public class SpeakerFlowSegmentRequestBuilder implements FlowCommandBuilder {
    ...
    private List<FlowSegmentRequestFactory> makePathRequests(
        ...
        for (PathSegment segment : path.getSegments()) {
            if (lastSegment == null) {
                if (doIngress) {
                    requests.addAll(makeSharedIngressRequests(context, flow, pathView));
    ...
    private List<FlowSegmentRequestFactory> makeSharedIngressRequests(
            CommandContext commandContext, Flow flow, FlowPathSpeakerView pathView) {
        List<FlowSegmentRequestFactory> requestFactories = new ArrayList<>();

        FlowSideAdapter ingressFlowSide = FlowSideAdapter.makeIngressAdapter(flow, pathView.getPath());
        if (pathView.getSharedIngressSegmentOuterVlanMatchStatus() != null) {
            requestFactories.add(makeSharedIngressSegmentOuterVlanMatchRequest(
                    commandContext, ingressFlowSide, pathView.getSharedIngressSegmentOuterVlanMatchStatus()));
        }
